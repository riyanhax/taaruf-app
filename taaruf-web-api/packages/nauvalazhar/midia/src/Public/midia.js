(function($, window, i) {
	$.fn.midia = function(options) {
		...
		this.each(function() {
			...
			$(this).attr('data-midia', id);
			let me = $('[data-midia="'+id+'"]');

			$("body").on("click", ".midia-close", function() {
				midia.close();
				return false;
			});
			...
			$("body").off('click', '.midia-delete').on("click", ".midia-delete", function() {
				let _this = $(this),
			...
			var midia = {
				open: function(_this_toggle) {
					...
					$("#midia-upload").off('click').on("click", function() {
						let _this = $(this);
					...
					$("#midia-search").off("keydown").on("keydown", function(e) {
						if(e.key == 'Enter') {
							$(".midia-loader #midia-files-loader").html("");
							limit = 0;
							showing = 0;
					...
					$("body").on("click", "#midia-reload", function() {
						$(".midia-loader #midia-files-loader").html("");
						limit = 0;
						showing = 0;
						load_files();
					...
					$("body").on("click", ".midia-files .midia-item .midia-item-inner", function() {
					...
					$("body").off("dblclick", ".midia-files .midia-item .midia-item-inner").on("dblclick", ".midia-files .midia-item .midia-item-inner", function() {
						rename_file();
						return false;
					});
					...
					$("body").off("click", ".midia-files .midia-item .midia-choose").on("click", ".midia-files .midia-item .midia-choose", function() {
						let _this = $(this),
						...
						if (window.tinyMCEPopup) {
							...
							$("#" + _this_toggle.data(options.data_target)).val(file_name);
							if($("#" + _this_toggle.data(options.data_preview)).length) {
								$("#" + _this_toggle.data(options.data_preview)).attr('src', url);
							}
							console.log(me.data('preview'))
					...
					$("body").on("click", "#midia-loadmore", function() {
						let _this = $(this);
					...
					$("body").on("click", ".midia-disabled", function() {
						return false;
					});
				},
				close: function() {
					$(".midia-wrapper").remove();
					$("body").css({
						overflow: 'initial'
					});
